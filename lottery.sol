...
contract Lottery {
    ...
    function pickWinner() public {
        ...
        emit Winner(winner, winAmount); // Emit an event to signal that a winner has been picked
        winner.transfer(winAmount); // Transfers the winAmount to the winner's address
    }

    // Event that is emitted when a winner is picked
    event Winner(address payable winner, uint winAmount);
}